...
from netio import TcpConnection, UdpConnection
...
def choose_protocol():
    while True:
        print("1 - TCP")
        print("2 - UDP")
        protocol_input = input("Выберите протокол подключения: ").strip()

        if protocol_input == "" or protocol_input == "1":
            return TCP
        elif protocol_input == "2":
            return UDP
        elif protocol_input.lower() == QUIT_STR:
            quit_program()
        elif protocol_input.lower() == BACK_STR:
            return None
        else:
            cnsl.log(LogTag.WARNING, "Некорректный ввод.")
...
def handle_server_udp(ip, port):
    try:
        ...
        conn = UdpConnection(sock, (ip, port), 3)
        while True:
            try:
                if not communicate_with_server(conn):
                    return
