...
def prepare_load(conn, tracker: ProgressTracker, byte_count, file_size):
    conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
    tracker.bytes = byte_count
    tracker.start(file_size)

def end_load(conn, tracker: ProgressTracker):
    tracker.stop()
    conn.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
...
def handle_sever_download(conn, filepath):
...
def handle_sever_upload(conn, filename):
    status = can_create_file(filename)
    netstat.send_status(conn, status)
    if status == netstat.OK:
        return download(conn, filename)

def handle_client_upload(conn, message):
    ...
    if status != netstat.OK:
        ...
        raise CommandCancel
    netio.w_line(conn, f"{netio.Commands.UPLOAD} {filename}")
    status = netstat.get_status(conn)
    if status != netstat.OK:
        if status == netstat.CONNECTION_CLOSED:
...
def handle_client_download(conn, message):
    filename, filepath, error = validate_download_command(message)
    if error:
        cnsl.log(LogTag.ERROR, error)
        raise CommandCancel
    netio.w_line(conn, f"{netio.Commands.DOWNLOAD} {filepath}")
    status = netstat.get_status(conn)
    if status != netstat.OK:
        if status == netstat.CONNECTION_CLOSED:
            raise ConnectionClosed
...
def receive_file_chunks(file, conn, total_size, tracker: ProgressTracker):
    bytes_left = total_size - tracker.bytes
    received = tracker.bytes
    while bytes_left > DEFAULT_CHUNK_SIZE:
        chunk = netio.r_exact(conn, DEFAULT_CHUNK_SIZE)
        if not chunk:
            return False
        file.write(chunk)
        received += len(chunk)
    ...
    if 0 < bytes_left <= DEFAULT_CHUNK_SIZE:
        chunk = netio.r_exact(conn, bytes_left)
        if chunk:
            file.write(chunk)
            received += len(chunk)
            bytes_left -= len(chunk)
...
def send_file_chunks(file, conn, chunk_size, file_size,  tracker: ProgressTracker):
    bytes_sent = tracker.bytes
    while bytes_sent < file_size:
        chunk = file.read(chunk_size)
        if not chunk:
            break
        netio.w_data(conn, chunk)
...
def download(conn, filename):
    netio.w_long(conn, DEFAULT_CHUNK_SIZE)
    file_size = netio.r_long(conn)
    if not file_size:
    ...
    netio.w_long(conn, received)

    tracker = ProgressTracker()
    prepare_load(conn, tracker, received, file_size)
    try:
        with prepare_file(filepath, received) as file:
            success = receive_file_chunks(file, conn, file_size, tracker)
    finally:
        end_load(conn, tracker)

    if success:
        finalize_download(filepath, final_path)
        speed = bytes.calculate_speed(start_time, time.time(), file_size - start_size)
...
def upload(conn, filepath, filename):
    chunk_size = netio.r_long(conn)
    if chunk_size is None:
        msg = f"Не удалось получить размер чанка."
        cnsl.log(LogTag.INFO, msg)
        return msg
    file_size = os.path.getsize(filepath)
    netio.w_long(conn, file_size)
    bytes_sent = netio.r_long(conn)
    if bytes_sent is None:
        msg = f"Не удалось получить количество переданых данных."
        cnsl.log(LogTag.INFO, msg)
        return msg
    cnsl.log(LogTag.INFO, f"Отправка файла '{filename}' ({bytes.human_readable_size(file_size-bytes_sent)})...")
    start_time = time.time()
    start_offset = bytes_sent

    tracker = ProgressTracker()
    prepare_load(conn, tracker, bytes_sent, file_size)
    try:
    ...
    if success:
        speed = bytes.calculate_speed(start_time, time.time(), file_size - start_offset)
