...
$stage = Join-Path $stageRoot "apcu_lab2_memory"
...
Copy-Item (Join-Path $PSScriptRoot "create_project.tcl"), (Join-Path $PSScriptRoot "memory_transfer.xdc") "$stageFull\vivado"
...
$runDir = Join-Path $stageFull "vivado\memory_transfer_vivado\memory_transfer.runs\synth_1"
Copy-Item -Force (Join-Path $runDir "runme.log") $outDir
Copy-Item -Force (Join-Path $runDir "memory_transfer_rtl_view_top_utilization_synth.rpt") $outDir
Copy-Item -Force (Join-Path $runDir "memory_transfer_rtl_view_top.dcp") $outDir
Copy-Item -Force (Join-Path $stageFull "vivado\memory_transfer_vivado\memory_transfer_synth.dcp") $outDir
