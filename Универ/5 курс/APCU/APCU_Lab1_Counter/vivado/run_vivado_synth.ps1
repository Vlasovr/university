...
$stage = Join-Path $stageRoot "apcu_lab1_counter"
...
Copy-Item (Join-Path $PSScriptRoot "create_project.tcl"), (Join-Path $PSScriptRoot "mod11_counter.xdc") "$stageFull\vivado"
...
$runDir = Join-Path $stageFull "vivado\mod11_counter_vivado\mod11_counter.runs\synth_1"
Copy-Item -Force (Join-Path $runDir "runme.log") $outDir
Copy-Item -Force (Join-Path $runDir "mod11_counter_rtl_view_top_utilization_synth.rpt") $outDir
Copy-Item -Force (Join-Path $runDir "mod11_counter_rtl_view_top.dcp") $outDir
Copy-Item -Force (Join-Path $stageFull "vivado\mod11_counter_vivado\mod11_counter_synth.dcp") $outDir
